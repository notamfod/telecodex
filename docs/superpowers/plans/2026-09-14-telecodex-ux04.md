...
Ограничения: браузерные проверки используют подменённые API; реальный Telegram WebView и мобильный возврат не проверены. Текст ожидаемого вопроса в текущей модели отсутствует, поэтому показывается подтверждённый вид ожидания и ссылка в переписку. UX-04 не закоммичен, не опубликован и не установлен в сервис.
