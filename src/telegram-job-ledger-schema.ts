...
export function initializeTelegramJobSchema(database: Database.Database): void {
  database.transaction(() => {
    ...
    if (version === 0) {
      ...
      for (const schema of Object.values(TABLES_V9)) database.exec(schema);
      ...
      const count = database.prepare("SELECT count(*) AS count FROM topic_resume_attempts")
        .pluck().get();
      if (count !== 0) throw new Error("Cannot migrate active Telegram topic resumes");
