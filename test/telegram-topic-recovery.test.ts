...
describe("Telegram topic recovery", () => {
  ...
  test.each([
    ...
    ["empty thread id", (value: ReturnType<typeof fixture>) => { value.job = job({ threadId: "" }); value.thread = thread({ id: "" }); }],
    ["blank thread title", (value: ReturnType<typeof fixture>) => { value.thread = thread({ title: " \n\t " }); }],
    ["empty thread cwd", (value: ReturnType<typeof fixture>) => { value.thread = thread({ cwd: "" }); }],
    ["relative thread cwd", (value: ReturnType<typeof fixture>) => { value.thread = thread({ cwd: "work/telecodex" }); }],
    ["root thread cwd", (value: ReturnType<typeof fixture>) => { value.thread = thread({ cwd: "/" }); }],
    ["absent response plan", (value: ReturnType<typeof fixture>) => { value.job = job({ responsePlan: undefined }); }],
    ["zero job version", (value: ReturnType<typeof fixture>) => { value.job = job({ version: 0 }); }],
